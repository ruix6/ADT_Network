...
from task2_regression.models.fcnn_env import FCNN, pearson_loss, pearson_metric, pearson_tf_non_averaged
...
if __name__ == "__main__":
   ...
   # Get the path to the config gile
    ...
    results_folder = os.path.join(experiments_folder, "results_fcnn_env")
    os.makedirs(results_folder, exist_ok=True)

    # create the model
    model = FCNN(window_length)
    model.compile(tf.keras.optimizers.Adam(learning_rate=1e-3), loss=pearson_loss, metrics=[pearson_metric])
    ...
    if only_evaluate:
        ...
        model.fit(
            ...
            callbacks=[
                ...
                tf.keras.callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=5, mode='auto', min_lr=0.0001)
            ],
